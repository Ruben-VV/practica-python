...
def extract_aeronet_data(inputfile):
    ...
    parse = lambda regdate, regtime: pd.datetime.strptime(' '.join([regdate,
                                                                    regtime]),
                                                          '%d:%m:%Y %H:%M:%S')
    #parse = lambda regdate,regtime: pd.datetime.strptime(regdate + ' ' + regtime, '%d:%m:%Y %H:%M:%S')

    df_wholefile = pd.read_csv(inputfile, delimiter=',', skiprows=4,
                               header=0, usecols=[0, 1, 12, 19],
                               parse_dates={'datetime': [0, 1]},
                               date_parser=parse)

    # Convertimos a mm el agua precipitable
    df_wholefile['Water(cm)'] = df_wholefile['Water(cm)']*10
