...
Efron, An Introduction to the Bootstrap. Chapman & Hall 1993
    ...
    if output == 'lowhigh':
        ...
        if nvals.ndim == 1:
          return abs(statfunction(data)-stat[nvals])[np.newaxis].T, stat
        else:
          return abs(statfunction(data)-stat[(nvals, np.indices(nvals.shape)[1:])])[np.newaxis].T, stat
    else:
        raise ValueError("Output option {0} is not supported.".format(output))
...
if __name__ == "__main__":
    ...
    ci_mean, stat_mean = ci(x, np.mean, method='pi')
